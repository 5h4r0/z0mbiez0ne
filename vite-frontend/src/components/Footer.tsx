import { Link } from 'react-router';

function ZombieLogoFooter() {
  return (
    <Link to="/" style={{ textDecoration: 'none', display: 'inline-block', marginBottom: '12px' }}>
      <span
        style={{
          fontFamily: "'bebas-neue-regular', serif",
          fontWeight: 700,
          fontSize: '1rem',
          color: 'var(--color-gold)',
          letterSpacing: '0.05em',
        }}
      >
        THE <span style={{ color: 'var(--color-gold)' }}>Z</span>
        <span style={{ color: 'var(--color-red)' }}>0</span>
        <span style={{ color: 'var(--color-gold)' }}>MBIE Z</span>
        <span style={{ color: 'var(--color-red)' }}>0</span>
        <span style={{ color: 'var(--color-gold)' }}>NE</span>
      </span>
    </Link>
  );
}

const footerLinkStyle: React.CSSProperties = {
  color: 'var(--color-text-muted)',
  textDecoration: 'none',
  fontSize: '0.875rem',
  transition: 'color 0.2s',
  display: 'block',
  marginBottom: '8px',
};

function FooterLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link
      to={to}
      style={footerLinkStyle}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLAnchorElement).style.color = 'var(--color-text)';
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLAnchorElement).style.color = 'var(--color-text-muted)';
      }}
    >
      {children}
    </Link>
  );
}

function SocialIcon({ label, path }: { label: string; path: string }) {
  return (
    <button
      type="button"
      aria-label={label}
      style={{
        background: 'none',
        border: 'none',
        cursor: 'pointer',
        padding: 0,
        color: 'var(--color-text-muted)',
        transition: 'color 0.2s',
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLButtonElement).style.color = 'var(--color-text)';
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLButtonElement).style.color = 'var(--color-text-muted)';
      }}
    >
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d={path} />
      </svg>
    </button>
  );
}

const FB_PATH =
  'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z';
const IG_PATH =
  'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z';
const YT_PATH =
  'M23.495 6.205a3.007 3.007 0 0 0-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 0 0 .527 6.205a31.247 31.247 0 0 0-.522 5.805 31.247 31.247 0 0 0 .522 5.783 3.007 3.007 0 0 0 2.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 0 0 2.088-2.088 31.247 31.247 0 0 0 .5-5.783 31.247 31.247 0 0 0-.5-5.805zM9.609 15.601V8.408l6.264 3.602z';

export default function Footer() {
  return (
    <footer style={{ backgroundColor: '#080808', borderTop: '1px solid var(--color-border)' }}>
      <div
        style={{
          maxWidth: '1400px',
          margin: '0 auto',
          padding: '64px 24px 32px',
          display: 'grid',
          gridTemplateColumns: 'repeat(4, 1fr)',
          gap: '48px',
        }}
      >
        {/* Col 1 — Brand */}
        <div>
          <ZombieLogoFooter />
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '6px', lineHeight: 1.6 }}>
            Parc à zombies réservé aux vivants
          </p>
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '6px' }}>
            Nîmes, France, avant l'apocalypse
          </p>
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '16px' }}>
            Visitez nos pages sur les réseaux
          </p>
          <div style={{ display: 'flex', gap: '16px' }}>
            <SocialIcon label="Facebook" path={FB_PATH} />
            <SocialIcon label="Instagram" path={IG_PATH} />
            <SocialIcon label="YouTube" path={YT_PATH} />
          </div>
        </div>

        {/* Col 2 — Navigation */}
        <div>
          <h3
            style={{
              fontFamily: 'Montserrat, sans-serif',
              fontWeight: 700,
              fontSize: '0.875rem',
              color: 'var(--color-text)',
              marginBottom: '16px',
              letterSpacing: '0.08em',
              textTransform: 'uppercase',
            }}
          >
            Navigation
          </h3>
          <FooterLink to="/les-epreuves">Activités</FooterLink>
          <FooterLink to="/sessions">Sessions</FooterLink>
          <FooterLink to="/categories-epreuves">Catégories</FooterLink>
          <FooterLink to="/plan">Plan</FooterLink>
          <FooterLink to="/tarifs">Tarifs</FooterLink>
          <FooterLink to="/faq">FAQ</FooterLink>
        </div>

        {/* Col 3 — Légal */}
        <div>
          <h3
            style={{
              fontFamily: 'Montserrat, sans-serif',
              fontWeight: 700,
              fontSize: '0.875rem',
              color: 'var(--color-text)',
              marginBottom: '16px',
              letterSpacing: '0.08em',
              textTransform: 'uppercase',
            }}
          >
            Au cas où
          </h3>
          <FooterLink to="/mentions-legales">Mentions Légales</FooterLink>
          <FooterLink to="/cgu">CGU</FooterLink>
          <FooterLink to="/confidentialite">Confidentialité</FooterLink>
          <FooterLink to="/contact">Contact</FooterLink>
        </div>

        {/* Col 4 — Newsletter */}
        <div>
          <h3
            style={{
              fontFamily: 'Montserrat, sans-serif',
              fontWeight: 700,
              fontSize: '0.875rem',
              color: 'var(--color-text)',
              marginBottom: '16px',
              letterSpacing: '0.08em',
              textTransform: 'uppercase',
            }}
          >
            Newsletter
          </h3>
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '12px' }}>Abonnez-vous</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label htmlFor="newsletter-email" style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>
              Email
            </label>
            <input
              id="newsletter-email"
              type="email"
              placeholder="votre@email.fr"
              style={{
                backgroundColor: 'var(--color-surface)',
                border: '1px solid var(--color-border)',
                borderRadius: '4px',
                padding: '8px 12px',
                color: 'var(--color-text)',
                fontSize: '0.875rem',
                outline: 'none',
                width: '100%',
              }}
            />
            <button
              type="button"
              style={{
                backgroundColor: 'var(--color-red)',
                color: '#fff',
                border: 'none',
                borderRadius: '4px',
                padding: '8px 16px',
                fontSize: '0.875rem',
                fontWeight: 600,
                cursor: 'pointer',
                letterSpacing: '0.04em',
                transition: 'background-color 0.2s',
              }}
              onMouseEnter={(e) => {
                (e.currentTarget as HTMLButtonElement).style.backgroundColor = 'var(--color-red-hover)';
              }}
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLButtonElement).style.backgroundColor = 'var(--color-red)';
              }}
            >
              S'abonner
            </button>
          </div>
        </div>
      </div>

      <div
        style={{
          borderTop: '1px solid var(--color-border)',
          textAlign: 'center',
          padding: '16px 24px',
          fontSize: '0.75rem',
          color: 'var(--color-text-muted)',
        }}
      >
        © {new Date().getFullYear()} The Z0mbie Z0ne. Tous droits réservés.
      </div>

      <style>{`
        @media (max-width: 1024px) {
          footer > div:first-child { grid-template-columns: repeat(2, 1fr) !important; }
        }
        @media (max-width: 640px) {
          footer > div:first-child { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </footer>
  );
}
