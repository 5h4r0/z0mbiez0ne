  ...
  return (
    <article className="bg-(--color-surface) border border-(--color-border) rounded-lg overflow-hidden flex flex-col transition-colors duration-200 hover:bg-(--color-surface-hover)">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <Link
          to={`/${category.slug}`}
          className="border border-(--color-text) hover:border-(--color-red) text-(--color-text) hover:text-(--color-red) px-4 py-1.5 rounded no-underline text-[0.8rem] font-semibold tracking-wider uppercase text-center transition-colors duration-200"
        >
          Voir la catégorie
        </Link>
      </div>
