...
export default function SessionCard({ session }: Props) {
  ...
  return (
    <article className="bg-(--color-surface) border border-(--color-border) rounded-lg overflow-hidden transition-colors duration-200 hover:bg-(--color-surface-hover)">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <Link
            to={href}
            className="bg-(--color-red) hover:bg-(--color-red-hover) text-white px-4 py-1.5 rounded no-underline text-[0.8rem] font-semibold tracking-wider uppercase transition-colors duration-200"
          >
            Réserver
          </Link>
        </div>
